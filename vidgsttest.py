...
# class Renderer():
    ...
    OVERLAY_CAPS = "video/x-raw,format=RGBA,bpp=32,depth=32,width=%d,height=%d,red_mask=-16777216,green_mask=16711680,blue_mask=65280,alpha_mask=255,endianness=4321,framerate=1/30" % (OVERLAY_FRAME_WIDTH, OVERLAY_FRAME_HEIGHT)

    PIPELINE_SIMPLE = "appsrc name=overlay do-timestamp=true caps=\"%s\" ! eglglessink" % (OVERLAY_CAPS)
    PIPELINE_MIX = "videotestsrc ! videomixer name=mix ! eglglessink         appsrc name=overlay do-timestamp=true caps=\"%s\" ! mix." % (OVERLAY_CAPS)

    def __init__(self):
        # self.pipeline = Gst.parse_launch(self.PIPELINE_SIMPLE)
        self.pipeline = Gst.parse_launch("videotestsrc pattern=smpte ! video/x-raw,width=512,height=384 ! gtksink")
