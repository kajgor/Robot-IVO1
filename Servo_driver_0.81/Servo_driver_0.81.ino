...
=============================================
History
20180203: v0.81 - HeartBeat added, max speed
                  "wobbling" fixed
=============================================
*/
...
/*********************************************************/
 ...
 void Set_Motor(boolean RL, boolean Stall_RL, boolean Stall_OT)
 {
    ...
    if ( Req_Rpm > 0 && Stall_RL == false) {
      ...
      if (Rpm_prev[RL] == 0) {
        if (Rpm[RL] > 0) {       // reduce power once gear runs
//          Pwr[RL] -= 50;
          Pwr[RL] = int(Pwr[RL] * 0.5);
          Rpm[RL] /= 2;
        }
        else {                   // raise power power if gear does not run
          Pwr[RL] += 200;
        }
      }
...
boolean ReadSerialData(int Buf_len)
{
  ...
  if ( StartChar == 255 ) {
    ...
    int HeartBeat_rcvd      = int(Serial.read()); // 7
